...
export default function Creative({ data, onChange }) {
  if (!data) return null;
  ...
  return (
    <div id="resume-document" className="resume-document creative">
      <div className="cre-sidebar" style={{ backgroundColor: 'var(--accent-color, #2c3e50)' }}>
        <div className="cre-header">
          ...
          <EditableText as="p" className="cre-title" value={data.personal?.title} onChange={(v) => updateField('personal.title', v)} placeholder="Job Title" style={{ color: 'rgba(255,255,255,0.8)' }} />
        </div>
        
        <div className="cre-contact">
          <div style={{ marginBottom: '3mm' }}><EditableText value={data.personal?.email} onChange={(v) => updateField('personal.email', v)} placeholder="Email" /></div>
          <div style={{ marginBottom: '3mm' }}><EditableText value={data.personal?.phone} onChange={(v) => updateField('personal.phone', v)} placeholder="Phone" /></div>
          <div style={{ marginBottom: '3mm' }}><EditableText value={data.personal?.location} onChange={(v) => updateField('personal.location', v)} placeholder="Location" /></div>
        </div>
          ...
          <div className="cre-section">
            ...
            <div className="cre-skills">
              {data.skills.map((skill, i) => (
                <span key={i} className="cre-skill-tag" style={{ backgroundColor: 'rgba(0,0,0,0.2)' }}>
                    ...
                    <SortableItem key={exp.id} id={exp.id}>
                      <div className="cre-item">
                        <div className="cre-item-header">
                          ...
                          <span className="cre-dates" style={{ color: 'var(--accent-color, #1abc9c)' }}>
                        ...
                        <div className="cre-company"><EditableText value={exp.company} onChange={(v) => updateField(`experience.${i}.company`, v)} placeholder="Company" /></div>
                        <ul>
                  ...
                  {educationItems.map((edu, i) => (
                    <SortableItem key={edu.id} id={edu.id}>
                      <div className="cre-item">
                        ...
                        <div className="cre-company"><EditableText value={edu.degree} onChange={(v) => updateField(`education.${i}.degree`, v)} placeholder="Degree" /></div>
                      </div>
                    </SortableItem>
                  ))}
                </SortableContext>
