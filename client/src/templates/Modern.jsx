...
export default function Modern({ data, onChange }) {
  if (!data) return null;
  ...
  return (
    <div id="resume-document" className="resume-document modern">
      <div className="mod-header">
        <div className="mod-header-left">
          ...
          <EditableText as="p" className="mod-title" value={data.personal?.title} onChange={(v) => updateField('personal.title', v)} placeholder="Job Title" />
        </div>
        <div className="mod-header-right">
          <div><EditableText value={data.personal?.email} onChange={(v) => updateField('personal.email', v)} placeholder="Email" /></div>
          <div><EditableText value={data.personal?.phone} onChange={(v) => updateField('personal.phone', v)} placeholder="Phone" /></div>
          <div><EditableText value={data.personal?.location} onChange={(v) => updateField('personal.location', v)} placeholder="Location" /></div>
        </div>
      </div>
      ...
      <div className="mod-content">
        {data.summary !== undefined && (
          <div className="mod-section">
            <h2 className="mod-section-title">Summary</h2>
            <EditableText as="p" className="mod-text" value={data.summary} onChange={(v) => updateField('summary', v)} placeholder="Summary..." />
          </div>
        )}
                    ...
                    <SortableItem key={exp.id} id={exp.id}>
                      <div className="mod-item">
                        <div className="mod-item-header">
                          <span className="mod-role"><EditableText value={exp.role} onChange={(v) => updateField(`experience.${i}.role`, v)} placeholder="Role" /></span>
                        ...
                        <div className="mod-company"><EditableText value={exp.company} onChange={(v) => updateField(`experience.${i}.company`, v)} placeholder="Company" /></div>
        ...
        <div className="mod-bottom-grid">
                ...
                <SortableContext items={educationItems.map(i => i.id)} strategy={verticalListSortingStrategy}>
                  {educationItems.map((edu, i) => (
                    <SortableItem key={edu.id} id={edu.id}>
                      <div className="mod-item">
                        <div className="mod-role"><EditableText value={edu.degree} onChange={(v) => updateField(`education.${i}.degree`, v)} placeholder="Degree" /></div>
                        <div className="mod-company">
                          <EditableText value={edu.institution} onChange={(v) => updateField(`education.${i}.institution`, v)} placeholder="Institution" /> • 
                          <EditableText value={edu.year} onChange={(v) => updateField(`education.${i}.year`, v)} placeholder="Year" />
                        </div>
                      </div>
                    </SortableItem>
                  ))}
          ...
          {data.skills && data.skills.length > 0 && (
            <div className="mod-section">
              <h2 className="mod-section-title">Skills</h2>
              <div className="mod-skills-container">
